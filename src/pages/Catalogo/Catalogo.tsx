...
export function Catalogo() {
  ...
  async function handleFavoritar(veiculoId: string) {
    if (favoritos.includes(veiculoId)) {
      await api.delete(`/favoritos/${veiculoId}`, { data: { sessionId } })
      setFavoritos((prev) => prev.filter((id) => id !== veiculoId))
    } else {
      await api.post('/favoritos', { sessionId, veiculoId })
  ...
  function handleLimpar() {
    const filtrosLimpos = { marca: '', modelo: '', preco_max: '', combustivel: '', cambio: '' }
  ...
  return (
    <div className={styles.container}>
      ...
      <MarcasCarrossel
        marcaSelecionada={filtros.marca}
        onSelecionarMarca={(nome) => {
          const novaMarca = filtros.marca === nome ? '' : nome
          setFiltros((prev) => ({ ...prev, marca: novaMarca }))
          const params: Record<string, string> = {}
          if (novaMarca) params['marca'] = novaMarca
              ...
              <div className={styles.precos}>
                {faixasPreco.map((preco) => (
                  <button
                    key={preco.value}
                    type="button"
                    onClick={() => setFiltros((p) => ({ ...p, preco_max: preco.value }))}
            ...
            <div className={styles.gridSelects}>
              <div className={styles.grupo}>
                ...
                <select
                  ...
                  onChange={(e) => setFiltros((p) => ({ ...p, combustivel: e.target.value }))}
                  className={styles.input}
                >
